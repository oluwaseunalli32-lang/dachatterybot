...
SESSION_CONTENT = [
    ...
    # =====================================================
    # NEW CONTENT 1: Large Orders (with 2 Images)
    # =====================================================
    {
        "name": "Large Orders",
        ...
        "pairs": [
            (
                ...
                "Check out these large INR orders on the Buy screen! Don't miss out.",
            ),
        ],
        "final_a": (
            "🔤🔤 Please check your account balance first. If there is a balance but no sales, check your UPI tools.\n\n"
            "🛍Don’t always wait to sell your entire balance before buying. Once the token drops below 🤩🤩🤩🤩 Rupees, please buy it immediately! Keep it above 🤩🤩🤩🤩 Rupees and always to sell quickly!🏷🏷🏷\n\n"
            "👑Contact :@jetlee261"
        ),
        "final_b": (
            "🚀 Don't wait! Large orders are being filled fast.\n\n"
            "📤Official link: https://wallet.paisa-base.com/register?inviteCode=phar6p"
        ),
    },
    # =====================================================
    # NEW CONTENT 2: Extra Rewards (with 1 Image)
    # =====================================================
    {
        "name": "Extra Rewards",
        ...
        "pairs": [
            (
                "💸💸💸Our app offers numerous extra rewards, allowing you to earn far more than other platforms.",
                "Deposit now and receive your bonus automatically the next day!💘💘",
            ),
        ],
        "final_a": (
            "💸💸💸Our app offers numerous extra rewards, allowing you to earn far more than other platforms. Deposit now and receive your bonus automatically the next day!💘💘\n\n"
            "📤Official link: https://wallet.paisa-base.com/register?inviteCode=phar6p"
        ),
        "final_b": (
            "📱 HR Contact: 🎉@jetlee261"
...
def load_group_ids():
    ...
    group_ids = []
    try:
        with GROUP_IDS_FILE.open("r", encoding="utf-8") as file:
            for line in file:
                line = line.strip()
                if not line:
                    continue
                try:
                    ...
                    logger.warning("Ignoring invalid group ID: %s", line)
    except OSError as error:
        logger.error("Could not read group ID file: %s", error)
    return group_ids
...
def save_group_id(chat_id):
    ...
    try:
        with GROUP_IDS_FILE.open("w", encoding="utf-8") as file:
            for group_id in group_ids:
                file.write(f"{group_id}\n")
        logger.info("Saved group ID: %s", chat_id)
    except OSError as error:
        logger.error("Could not save group ID %s: %s", chat_id, error)

# =========================================================
# SEND FUNCTIONS (Now support images)
# =========================================================

async def send_bot_a(chat_id, text, image_url=None):
    if APP_A is None:
        logger.error("Bot A is not initialized.")
        return False
    try:
        if image_url:
            await APP_A.bot.send_photo(
                chat_id=chat_id,
                photo=image_url,
                caption=text,
                parse_mode="HTML"
        ...
        logger.info("Bot A sent message to %s", chat_id)
        return True
    except Exception as error:
        logger.error("Bot A failed to send to %s: %s", chat_id, error)
        return False


async def send_bot_b(chat_id, text, image_url=None):
    if APP_B is None:
        logger.error("Bot B is not initialized.")
        return False
    try:
        if image_url:
            await APP_B.bot.send_photo(
                chat_id=chat_id,
                photo=image_url,
                caption=text,
                parse_mode="HTML"
        ...
        logger.info("Bot B sent message to %s", chat_id)
        return True
    except Exception as error:
        logger.error("Bot B failed to send to %s: %s", chat_id, error)
        return False
...
async def run_session(chat_id, test_mode=False):
    logger.info("=" * 40)
    logger.info("SESSION STARTED | GROUP %s | TEST=%s", chat_id, test_mode)
    logger.info("=" * 40)
    ...
    if test_mode:
        number_of_pairs = min(1, len(content_set["pairs"]))
    ...
    for index, (message_a, message_b) in enumerate(pairs):
        ...
        await send_bot_a(chat_id, message_a, img_a)
        
        wait_time = random.randint(wait_min, wait_max)
        logger.info("Group %s: waiting %s seconds before Bot B.", chat_id, wait_time)
        await asyncio.sleep(wait_time)

        await send_bot_b(chat_id, message_b, img_b)

        if index < len(pairs) - 1:
            wait_time = random.randint(wait_min, wait_max)
            logger.info("Group %s: waiting %s seconds before next pair.", chat_id, wait_time)
            await asyncio.sleep(wait_time)

    logger.info("Group %s: sending final session messages.", chat_id)
    
    await send_bot_a(chat_id, content_set["final_a"])
    await asyncio.sleep(5)
    await send_bot_b(chat_id, content_set["final_b"])

    logger.info("=" * 40)
    logger.info("SESSION COMPLETED | GROUP %s", chat_id)
    logger.info("3-hour countdown can now begin.")
    logger.info("=" * 40)
...
async def group_session_loop(chat_id):
    logger.info("Starting recurring cycle for group %s", chat_id)
    try:
        await run_session(chat_id, test_mode=False)
        while True:
            logger.info("Group %s: session finished. Starting 3-hour countdown.", chat_id)
            await asyncio.sleep(SESSION_INTERVAL)
            logger.info("Group %s: 3-hour countdown completed.", chat_id)
            await run_session(chat_id, test_mode=False)
    except asyncio.CancelledError:
        logger.info("Recurring cycle cancelled for group %s", chat_id)
        raise
    except Exception as error:
        logger.exception("Recurring cycle failed for group %s: %s", chat_id, error)
    finally:
        current_task = asyncio.current_task()
        if GROUP_SESSION_TASKS.get(chat_id) is current_task:
            GROUP_SESSION_TASKS.pop(chat_id, None)
        logger.info("Recurring cycle stopped for group %s", chat_id)

# =========================================================
# START / STOP GROUP SESSION
# =========================================================

def start_group_session(chat_id):
    existing_task = GROUP_SESSION_TASKS.get(chat_id)
    if existing_task:
        if not existing_task.done():
            return False
    task = asyncio.create_task(group_session_loop(chat_id))
    GROUP_SESSION_TASKS[chat_id] = task
    logger.info("Created session task for group %s", chat_id)
    return True
...
async def stop_group_session(chat_id):
    ...
    if task.done():
        GROUP_SESSION_TASKS.pop(chat_id, None)
        return False
    logger.info("Cancelling session for group %s", chat_id)
    task.cancel()
    try:
        await task
    except asyncio.CancelledError:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type not in ("group", "supergroup"):
        if update.message:
            await update.message.reply_text("❌ Please use /start inside a group.")
        return
    chat_id = chat.id
    save_group_id(chat_id)
    started = start_group_session(chat_id)
    if started:
        if update.message:
            await update.message.reply_text("✅ Session cycle started.\n\nThe first promotional session will begin now.\nThe 3-hour countdown starts only after the session completely finishes.")
    else:
        if update.message:
            await update.message.reply_text("ℹ️ A session cycle is already active in this group.")

async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    if chat is None: return
    chat_id = chat.id
    stopped = await stop_group_session(chat_id)
    if update.message:
        if stopped:
            await update.message.reply_text("🛑 Session cycle stopped.")
        else:
            await update.message.reply_text("ℹ️ No active session cycle was found.")
...
async def test_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type not in ("group", "supergroup"):
        if update.message:
            await update.message.reply_text("❌ Please use /test inside a group.")
        return
    chat_id = chat.id
    existing_task = GROUP_SESSION_TASKS.get(chat_id)
    if existing_task and not existing_task.done():
        if update.message:
            await update.message.reply_text("⚠️ A recurring session is already active in this group.")
        return
    if update.message:
        await update.message.reply_text("🧪 One-time test session starting...")
    try:
        await run_session(chat_id, test_mode=True)
        if update.message:
            await update.message.reply_text("✅ One-time test completed.\n\nThe 3-hour recurring cycle was not started.")
    except asyncio.CancelledError:
        raise
    except Exception as error:
        logger.exception("One-time test failed: %s", error)
        if update.message:
            await update.message.reply_text("❌ Test failed. Check the server logs.")
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if task and not task.done():
        status = "🟢 ACTIVE\n\nThe recurring session cycle is active.\n\nSequence:\nSession → final CTA → 3-hour wait → next session."
    else:
